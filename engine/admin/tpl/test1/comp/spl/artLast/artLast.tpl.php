...
<script type="text/javascript">
    var artLastData = {
        ...
        artLast: <?= self::get('artLast') ?>,
        contid: <?= self::get('contId') ?>
    };

    var contrName = artLastData.contid;
    var callType = 'comp';
    utils.setType(callType);
    utils.setContr(contrName);
    HAjax.setContr(contrName);

    var artLastMvc = (function () {
        var options = {};
        var tree = {};
        ...
        function routeTreeDbClick(pTreeId, pTree) {
            $.fancybox.close();
            artLastData.tplUrl = utils.getTreeUrlTpl(pTree, pTreeId);
            $('#' + options.tplUrlText).html(artLastData.tplUrl);
        ...
        function initTrees() {
            dhtmlxInit.init({
                'contTree':{
                    tree:{
                        id:'contDiv', json:artLastData.contTree
                    }, // tree
                    checkbox:true
                }, // contTree
                'routeTree':{
                    tree:{
                        id:'routeBox', json:artLastData.routeTree
                    }, // tree
            ...
            for (var i in artLastData.artLast) {
                var id = artLastData.artLast[i];
