...
<script type="text/javascript">
    var artPopularData = {
        ...
        artPopular: <?= self::get('artPopular') ?>,
        contid: <?= self::get('contId') ?>
    };

    var contrName = artPopularData.contid;
    var callType = 'comp';
    utils.setType(callType);
    utils.setContr(contrName);
    HAjax.setContr(contrName);

    var artPopularMvc = (function () {
        var options = {};
        var tree = {};
        ...
        function routeTreeDbClick(pTreeId, pTree) {
            $.fancybox.close();
            artPopularData.tplUrl = utils.getTreeUrlTpl(pTree, pTreeId);
            $('#' + options.tplUrlText).html(artPopularData.tplUrl);
        ...
        function initTrees() {
            dhtmlxInit.init({
                'contTree':{
                    tree:{
                        id:'contDiv', json:artPopularData.contTree
                    }, // tree
                    checkbox:true
                }, // contTree
                'routeTree':{
                    tree:{
                        id:'routeBox', json:artPopularData.routeTree
                    }, // tree
            ...
            for (var i in artPopularData.artPopular) {
                var id = artPopularData.artPopular[i];
