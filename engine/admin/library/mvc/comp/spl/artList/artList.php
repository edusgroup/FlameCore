...
namespace admin\library\mvc\comp\spl\artList;
...
//use ORM\comp\spl\artList\artListCont as artListContOrm;

/**
 * Description of artList
 *
 * @author Козленко В.Л.
 */
class artList extends \core\classes\component\abstr\admin\comp {
    ...
    public function indexAction() {
        ...
        $artlist = (new artListOrm)->selectList('*', 'selContId', 'contId='.$contId);
        self::setJson('artlist', $artlist);

        self::setVar('contId', $this->contId);

        $artListProp = ( new artListPropOrm() )->selectFirst('');
        if ( $artListProp){
            self::setVar('itemsCount', $artListProp['itemsCount'] );
            self::setVar('memcacheCount', $artListProp['memcacheCount']);
            self::setVar('fileCount', $artListProp['fileCount']);
        } // if
    ...
    public function saveDataAction(){
        ...
        $artListOrm->delete('contId='.$contId);
        ...
        if ( $selData ){
            ...
            $artListOrm->insertMulti(['selContId' => $selData]);
            $artListOrm->update('contId='.$contId, 'contId=0');
        } // if selData
        ...
        ( new artListPropOrm() )->saveExt(['contId' => $contId], $saveData);
